QUnit.module('pointer2 border Focus 2', (hooks) => {
  ...
  QUnit.test('by default pointer2 border focus style is empty', (assert) => {
    ...
    assert.equal($pointer2.style.getPropertyValue('--tc-range-slider-pointer-border-focus'), '');
  });

  QUnit.test('provided pointer2 border focus style should be none', (assert) => {
    const $slider = document.querySelector('#slider-27');
    const $pointer2 = $slider.shadowRoot.querySelector('.pointer-2');
    assert.equal($pointer2.style.getPropertyValue('--tc-range-slider-pointer-border-focus'), '0');
  });
  ...
  QUnit.test('pointer2 border focus changed to "0 0 2px red" ---> style should change', (assert) => {
    ...
    const $pointer2 = $slider.shadowRoot.querySelector('.pointer-2');
    assert.equal($pointer2.style.getPropertyValue('--tc-range-slider-pointer-border-focus'), '1px solid red');
  });
