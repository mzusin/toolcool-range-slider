QUnit.module('Pointer2 Border Hover', (hooks) => {
  ...
  QUnit.test('by default pointer2 border hover style is empty', (assert) => {
    ...
    assert.equal($pointer2.style.getPropertyValue('--tc-range-slider-pointer-border-hover'), '');
  });

  QUnit.test('provided pointer2 border hover style should be none', (assert) => {
    const $slider = document.querySelector('#slider-27');
    const $pointer2 = $slider.shadowRoot.querySelector('.pointer-2');
    assert.equal($pointer2.style.getPropertyValue('--tc-range-slider-pointer-border-hover'), '0');
  });
  ...
  QUnit.test('pointer2 border hover changed to "0 0 2px red" ---> style should change', (assert) => {
    ...
    const $pointer2 = $slider.shadowRoot.querySelector('.pointer-2');
    assert.equal($pointer2.style.getPropertyValue('--tc-range-slider-pointer-border-hover'), '1px solid red');
  });
