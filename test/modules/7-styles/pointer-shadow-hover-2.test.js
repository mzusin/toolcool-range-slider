QUnit.module('Pointer2 Shadow Hover', (hooks) => {
  ...
  QUnit.test('by default pointer2 shadow hover style is empty', (assert) => {
    ...
    assert.equal($pointer2.style.getPropertyValue('--tc-range-slider-pointer-shadow-hover'), '');
  });

  QUnit.test('provided pointer2 shadow hover style should be none', (assert) => {
    const $slider = document.querySelector('#slider-26');
    const $pointer2 = $slider.shadowRoot.querySelector('.pointer-2');
    assert.equal($pointer2.style.getPropertyValue('--tc-range-slider-pointer-shadow-hover'), 'none');
  });
  ...
  QUnit.test('pointer2 shadow hover changed to "0 0 2px red" ---> style should change', (assert) => {
    ...
    const $pointer2 = $slider.shadowRoot.querySelector('.pointer-2');
    assert.equal($pointer2.style.getPropertyValue('--tc-range-slider-pointer-shadow-hover'), '0 0 2px red');
  });
