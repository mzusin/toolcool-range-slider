...
QUnit.module('Range Slider Round', () => {
  ...
  QUnit.test('default round should be 5', (assert) => {
    const $slider = document.querySelector('#slider-1');
    assert.equal($slider.round, 5);
  });

  QUnit.test('provided round should be 2', (assert) => {
  ...
  QUnit.test('if provided round is string it should be 5', (assert) => {
    const $slider = document.querySelector('#slider-84');
    assert.equal($slider.round, 5);
  });

  QUnit.test('if provided value is negative it should be 5', (assert) => {
  ...
  QUnit.test('1.55678 when round is 2 ---> should be equal 1.56', (assert) => {
    const $slider = document.querySelector('#slider-82'); // provided round is 2
    $slider.value = 1.55678;
    assert.equal($slider.value, 1.56);
  });
